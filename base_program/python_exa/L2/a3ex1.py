...
"""
拆分姓名函数
1. 将中文姓名拆分为姓氏和名字，仅支持单姓
2. 将中文姓名拆分为姓氏和名字，仅支持复姓
3. 将中文姓名拆分为姓氏和名字，支持单复姓, 支持女士姓名中包含丈夫姓氏并返回合并后的姓氏
"""
...
def split_name2(name):
    ...
    if len(name) >= 2 and name[:2] in compound_surnames:
        ...
        # 如果不是复姓，按单姓处理
        print(f"!!!姓名 '{name}' 不是复姓，按单姓处理!!!")
        surname, given_name = split_name1(name)
    
    return surname, given_name


def split_name3(name):
    '''将中文姓名拆分为姓氏和名字，支持单复姓, 支持女士姓名中包含丈夫姓氏并返回合并后的姓氏'''
    if len(name) < 2:
        return name, ""
...
def test_split_name():
    ...
    surname, given_name = split_name1(name)
    print(f"姓: {surname}")
    print(f"名: {given_name}")
    assert surname == "张" and given_name == "三丰"

    # 复姓测试
    name = "司马光"
    print(f"复姓测试: {name}")
    surname, given_name = split_name2(name)
    print(f"姓: {surname}")
    print(f"名: {given_name}")
    assert surname == "司马" and given_name == "光"

    # 单复姓测试
    name = "林郑月娥"
    print(f"单复姓测试: {name}")
    surname, given_name = split_name3(name)
    print(f"姓: {surname}")
    print(f"名: {given_name}")
    assert surname == "林郑" and given_name == "月娥"
    print("拆分姓名函数测试通过")
    print("==="*20)
...
def input_name():
    ...
    name = input("输入你的中文姓名: ")
    surname, given_name = split_name2(name)
    print(f"将中⽂姓名拆分为姓氏和名字，仅支持复姓，结果如下: ")
    print(f"姓: {surname}")
    print(f"名: {given_name}")
    print("---"*20)
    ...
    name = input("输入你的中文姓名:")
    surname, given_name = split_name3(name)
    print(f"将中⽂姓名拆分为姓氏和名字，支持单复姓, 支持女士姓名中包含丈夫姓氏并返回合并后的姓氏，结果如下: ")
    print(f"姓: {surname}")
    print(f"名: {given_name}")
    print("---"*20)
